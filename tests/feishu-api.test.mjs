...
test("unconfigured status exposes setup instructions and callback without secrets", async () => {
  for (const cookie of [cookieA, cookieB]) {
    ...
    assert.deepEqual(payload.missingEnvironment, ["INTEGRATION_ENCRYPTION_KEY"]);
    assert.deepEqual(payload.requiredEnvironment, ["INTEGRATION_ENCRYPTION_KEY"]);
    assert.deepEqual(payload.requiredAppFields, ["FEISHU_APP_ID", "FEISHU_APP_SECRET"]);
...
test("status uses the public HTTPS origin supplied by the reverse proxy", async () => {
  const { response, payload } = await request("/api/integrations/feishu/status", {
    ...
    headers: {
      ...
      "x-forwarded-host": "inspiration.example.test",
    },
  });
  assert.equal(response.status, 200, JSON.stringify(payload));
  assert.equal(payload.callbackUrl, "https://inspiration.example.test/api/integrations/feishu/oauth/callback");
});
