...
test("unconfigured integration returns an actionable status without secret values", async (t) => {
  ...
  assert.deepEqual(status.missingEnvironment, ["INTEGRATION_ENCRYPTION_KEY"]);
  assert.deepEqual(status.requiredEnvironment, ["INTEGRATION_ENCRYPTION_KEY"]);
  assert.deepEqual(status.requiredAppFields, ["FEISHU_APP_ID", "FEISHU_APP_SECRET"]);
  assert.ok(!JSON.stringify(status).includes("appSecret"));
});

test("configured server prompts each owner to save their own Feishu app credentials", async (t) => {
  const db = await databaseFixture(t);
  const integration = new FeishuIntegration(db, {
    appId: "",
    appSecret: "",
  ...
  assert.equal(status.status, "app_missing");
  assert.equal(status.appId, "");
  assert.deepEqual(status.missingEnvironment, []);
  assert.deepEqual(status.requiredAppFields, ["FEISHU_APP_ID", "FEISHU_APP_SECRET"]);
  assert.throws(
    () => integration.startOAuth("owner-a"),
    (error) => error.status === 409 && error.code === "FEISHU_APP_CREDENTIALS_REQUIRED",
  );
});

test("server app fallback is limited to legacy connected owners", async (t) => {
  const db = await databaseFixture(t);
  const timestamp = new Date(fixedNow).toISOString();
  const integration = new FeishuIntegration(db, {
    appId: "cli-server-legacy",
    appSecret: "server-legacy-secret",
    encryptionKey: "test-encryption-key-with-enough-entropy",
    publicBaseUrl: "https://inspiration.example.test",
  });
  assert.equal(integration.getStatus("new-owner").status, "app_missing");
  ...
  db.prepare(`INSERT INTO feishu_connections
    ...
    "ciphertext-access",
    "ciphertext-refresh",
    new Date(fixedNow + 3_600_000).toISOString(),
    timestamp,
    timestamp,
    timestamp,
  );
  const legacy = integration.getStatus("legacy-owner");
  assert.equal(legacy.status, "connected");
  assert.equal(legacy.appConfigured, true);
  assert.equal(legacy.appCredentialSource, "server_legacy");
});

test("owner app credentials are encrypted and used for OAuth and token exchange", async (t) => {
  const db = await databaseFixture(t);
  const requests = [];
  const integration = new FeishuIntegration(db, integrationOptions({
    ...
    fetchImpl: async (_url, options) => {
      ...
      return jsonResponse(tokenPayload("user-app"));
    },
  }));

  const saved = integration.configureAppCredentials("owner-a", {
    appId: "cli-owner-app-123456",
    appSecret: "owner-secret-value",
  });
  ...
  const row = db.prepare("SELECT * FROM feishu_app_credentials WHERE owner_id=?").get("owner-a");
  assert.equal(row.app_id, "cli-owner-app-123456");
  assert.ok(!row.app_secret_ciphertext.includes("owner-secret-value"));
  assert.equal(createSecretBox(integrationOptions().encryptionKey).decrypt(row.app_secret_ciphertext), "owner-secret-value");

  const status = integration.getStatus("owner-a");
  assert.equal(status.appConfigured, true);
  assert.equal(status.appCredentialSource, "user");
  assert.equal(status.appId, "cli-owne…");

  const started = integration.startOAuth("owner-a");
  const authorization = new URL(started.authorizationUrl);
  assert.equal(authorization.searchParams.get("client_id"), "cli-owner-app-123456");
  await integration.handleOAuthCallback("owner-a", {
    state: authorization.searchParams.get("state"),
    code: "user-app",
  });
  assert.equal(requests[0].client_id, "cli-owner-app-123456");
  assert.equal(requests[0].client_secret, "owner-secret-value");
});
